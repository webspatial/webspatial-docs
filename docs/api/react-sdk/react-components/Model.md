...
The APIs documented on this page require `@webspatial/react-sdk` version `1.7.0` or later. Individual playback features also depend on the WebSpatial Runtime version; see [Runtime support](#runtime-support).
:::
...
:::note[Playback is not the same as moving the model]
